...
init_cohort <- tibble(
  ...
  #init_cohort_nindivs = c(2.0,rep(.05, 9)),
  ...
  #init_cohort_bsw     = c(0.01,rep(0.2, 9)),
  ...
  #init_cohort_nsc     = c(0.04,rep(0.5, 9)),
)

init_soil <- tibble( 
  init_fast_soil_C    = 0.0,
  #init_fast_soil_C    = 0.5,  
  init_slow_soil_C    = 0.0,   
  #init_slow_soil_C    = 40.0,    
  init_Nmineral       = 0.015,  
  #init_Nmineral       = 0.5,  
  N_input             = 0.0008,
  #N_input             = 0.02  
...
biomeep_output_annual_tile |>
 ...
 group_by(group = cumsum(dplyr::lag(year == 541, default = 0))) |>
 #mutate(cumsum_CB = cumsum(WDgrow+WDrepr-WDmort-WDkill-m_turnover)) |>
  mutate(cumsum_CB = cumsum(WDgrow+WDrepr-WDmort-WDkill-m_turnover)) |>
  ungroup |>
  ggplot() + 
  geom_line(aes(x=year, y=cumsum_CB),col="#377EB8") + 
...
biomeep_output_annual_tile |>
  ...
  mutate(year = 1:960, AGcwood = (SapwoodC+WoodC)*0.75) |>
  select(year, AGcwood)  |>
  ggplot() + 
...
# FLUXES
## 10.Woody biomass growth - WBgrowth
biomeep_output_annual_cohorts |> 
  filter(PFT != 1) |>
  mutate(PFT=as.factor(PFT)) |>
  group_by(PFT,year) |>
...
biomeep_output_annual_tile |> 
  slice(510+1:nrow(biomeep_output_annual_tile)) |> 
  mutate(year = 1:960, nbp = GPP-Rauto-Rh) |>
  select(year, nbp) |>
  ggplot() + 
